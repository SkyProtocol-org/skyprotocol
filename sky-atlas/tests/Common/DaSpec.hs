...
daSpec = testGroup "simple tests for SkyDA"
  let da0 = runIdentity $ initDa daSchema0 committee0 :: SkyDa (HashRef Hash)
      ...
      Just (msg1b, proof1) = runIdentity $ getSkyDataProof (topic0, msg1i) da3 :: Maybe (LiftRef (HashRef Hash) BuiltinByteString, SkyDataProof Blake2b_256)
  in
  ...
  , testCase "No proof for (0,0) in empty Da" $ do
      let maybeProof0 = runIdentity $ getSkyDataProof (fromInt 0, fromInt 0) da0 :: Maybe (LiftRef (HashRef Hash) BuiltinByteString, SkyDataProof Blake2b_256)
  ...
  , let (da10, Just topic1) = runIdentity $ insertTopic topicSchema0 da3
        ...
        Just (msg3b, _proof3) = runIdentity $ getSkyDataProof (topic1, msg3i) da11 :: Maybe (LiftRef (HashRef Hash) BuiltinByteString, SkyDataProof Blake2b_256)
    in testCase "msg3 matches" $ do
      msg3b == LiftRef (digestRef msg3) @?= True

  , let (da10, Just topic1) = runIdentity $ insertTopic topicSchema0 da3
        (da11, Just msg3i) = runIdentity $ insertMessage timestamp1 msg3 topic1 da10
        Just (msg3b, proof3) = runIdentity $ getSkyDataProof (topic1, msg3i) da11 :: Maybe (LiftRef (HashRef Hash) BuiltinByteString, SkyDataProof Blake2b_256)
