...
typesSpec = testGroup "SkyBase" $
  [ testCase "builtins" $ do
        ...
        case result of
          Left (ErrorCall x)  -> do
            ...
            trace "FOOOOOO" $ -- dropped on the ground! :-(
              x @?= "PlutusTx.Builtins.Internal.error" -- unhappily uninformative
          Right val -> do
    ...
    , testCase "serialization 7" $
        ...
        PS.show (5::Integer, "23"::BuiltinString, hexB "0badf00d") @?= "(5,\"23\",0badf00d)"
  ...
  in
  [ testCase "lowestBitClear 1.2" $ do
...
testBitLogic :: (BitLogic a, FromInt a, Dato a, Eq a, GS.Show a, Arbitrary a) =>
  String -> (Integer -> a) -> (a -> Integer) -> a -> Integer -> Bool -> TestTree
