...
trieSpec = testGroup "Spec.TrieSpec" $
  ...
  in
  [ testCase "list I/O 1" $ ol [] `shouldBeHex` "000000"
  ...
  , testProperty "testing creation of random tries" $ \(listOfK :: [Bytes8]) ->
      let someTrie :: S = runIdentity $ ofList $ zip listOfK $ fmap PS.show listOfK in
      case listOfK of
        (k1 : k2 : _) -> 
          runIdentity (lookup k1 someTrie) == Just (PS.show k1) &&
          runIdentity (lookup k2 someTrie) == Just (PS.show k2)
        (k1 : _) ->
          runIdentity (lookup k1 someTrie) == Just (PS.show k1)
        _ -> True
  ...
  , testProperty "testing inserting and removing" $ \(listOfK :: [Bytes8]) ->
      ...
      case listOfK of
        (k1 : k2 : _) ->
          let newTrie = runIdentity $ remove k1 someTrie >>= remove k2 in
          isNothing (runIdentity (lookup k1 newTrie)) &&
          isNothing (runIdentity (lookup k2 newTrie)) &&
          let newTrie' = runIdentity $ insert (PS.show k1) k1 newTrie >>= insert (PS.show k2) k2 in
          runIdentity (lookup k1 newTrie') == Just (PS.show k1) &&
          runIdentity (lookup k2 newTrie') == Just (PS.show k2)
        (k1 : _) -> 
          let newTrie = runIdentity $ remove k1 someTrie in
          isNothing (runIdentity (lookup k1 newTrie)) &&
          let newTrie' = runIdentity $ insert (PS.show k1) k1 newTrie in
          runIdentity (lookup k1 newTrie') == Just (PS.show k1)
        _ -> True
