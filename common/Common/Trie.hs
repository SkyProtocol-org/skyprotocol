...
pattern TrieTop {trieTopHeight, trieTopNode} = TrieTop_ (trieTopHeight, trieTopNode)

-- used to signify to the compiler, that TrieTop pattern synonym is exhaustive
-- for pattern matches on TrieTop type
{-# COMPLETE TrieTop #-}
...
data TrieStep h k o
  = -- | take a left step with that other thing on the right
    LeftStep o
  | -- | take a right step with that other thing on the left
    RightStep o
...
instance
  ...
  FromByteString (TrieNodeF h k c t)
  where
...
instance Functor (TrieNodeF h k c) where
  fmap _ Empty = Empty
  fmap _ (Leaf c) = Leaf c
...
instance
  ...
  {-# INLINEABLE stepDown #-}
  stepDown step p@(TriePath h k m s) =
    return
...
instance
  ...
  refocus (h', k') z@(Zip node path@(TriePath h0 k0 _ _))
    ...
    -- refocus an empty zipper
    | h0 == (-1) =
        let l = bitLength k'
            m = lowBitsMask l
            p = TriePath h' k' m [] :: TriePath h k (TrieNodeRef r h k c)
                  ...
                  let childHeight = h - toInt bitsHeightMinus1 - 1
                      ...
                      nodeBits =
                        extractBitField
                          ...
                          bits
                      diffLength = bitLength (keyBits `logicalXor` nodeBits)
                   in if diffLength == 0
                        then -- Not so hard: if it was the same key all the way that matters
                          ...
                          let sameLength = comparableLength - diffLength
                              ...
                              branchStep =
                                if isBitSet (branchNodeHeight - h' - 1) k'
                                  then RightStep
                                  else LeftStep
                           in do
                                oldBranch <-
...
trieZipperFocusOnly (Zip f p@(TriePath h k _ _)) =
  let done h' f' = return $ Zip f' (TriePath h' (lowBitsMask 0) (lowBitsMask 0) [])
   in fr f >>= \case
        Empty -> done (-1) f
...
zipFocusLastLeaf z = zipUp stepUp z >>= descendRight
  where
    descendRight z'@(Zip node path) =
      fr node >>= \case
...
zipLastLeaf =
  zipFocusLastLeaf >=> \(Zip node (TriePath _ k _ _)) ->
    fr node >>= \case
