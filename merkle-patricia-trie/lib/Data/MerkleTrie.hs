...
prove :: (Show a, Show k, Show (TrieHeight k), TrieKey k) => k -> Trie k a -> Maybe (Digest SHA256, [Digest SHA256])
prove k t = do
  z <- focus k t
  ...
  where
    -- TODO don't like the fact, that there are 2 cases with undefined here, although logically I know, that they will not show up
    ...
    go :: (Show a, Show k, Show (TrieHeight k), TrieKey k) => (Digest SHA256, [Digest SHA256]) -> Trie k a -> (Digest SHA256, [Digest SHA256])
