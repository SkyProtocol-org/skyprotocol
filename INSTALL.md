...
You can install *Sky Protocol* with this one-liner:

    curl -L https://skyprotocol.org/install/sky-install | sh

This will make the command `sky` available to you, though you may have
...
If you'd rather use Docker than Nix, you can run:
    ...
    docker pull mukn/sky:latest

The command `sky` is available in that image,
but see [below](#Trying-it-out-with-Docker) on how to invoke it.

## Warnings and Prerequisites

1. Installing *Sky Protocol* currently requires about 4GB of RAM and about
...
3. Nix installs its packages under `/nix`. If you are on a machine with
   a small root partition, *Sky Protocol* and its dependencies may be too big.
...
6. If you're not using one of those architectures, then the installation
   script will build not only *Sky Protocol*, but also many of its dependencies.
   ...
   in the second step of the script:
        ...
        nix-env --cores 4 -f $nixpkgs -iA skyprotocol
...
If you mean to use *Sky Protocol* as a developer or end-user
but not modify the implementation itself (compiler, runtime, libraries),
...
Then, you can checkout the code of *Sky Protocol* (if you haven't already) with

    git clone https://github.com/skyprotocol-org/skyprotocol

Now you can `cd skyprotocol` and build the software deterministically with:
...
Or, once Nix is installed, you can define this `nixpkgs` variable, then run
    ...
    nix-env -f $nixpkgs -iA skyprotocol
...
with *Sky Protocol* and its associated libraries precompiled.
...
is better suited to work on *Sky Protocol* itself and can be unstable), use:
...
between sessions of running *Sky Protocol* inside docker.
...
## Installing *Sky Protocol* the Hard Way

If you are going to be modifying the implementation of *Sky Protocol*,
...
then you will have to build and install *Sky Protocol* the Hard Way:
installing GCC, on top of it Gambit Scheme, on top of it Gerbil Scheme,
on top of it a bunch of libraries, and finally, *Sky Protocol*.
...
might *not* be recent enough version to build *Sky Protocol*.
...
a recent enough version of Gerbil to build *Sky Protocol*, at which point you will have
...
### Last but not least, build *Sky Protocol*
...
## Testing *Sky Protocol*

Once you have built and installed *Sky Protocol*, you should be all ready to go.
...
you shouldn't need to build *Sky Protocol* as above to test it.

## Run it

Once you built the software with `./build.ss`, you can run it with `./sky`.

## Install it

If you use Nix, it will have already installed *Sky Protocol*.

Otherwise, you can copy the `./sky` binary anywhere on your machine.
...
for the user who compiles *Sky Protocol*.
...
To see how *Sky Protocol* is installed by its Nix package, see
[skyprotocol.nix](https://github.com/MuKnIO/nixpkgs/blob/devel/pkgs/development/compilers/gerbil/skyprotocol.nix)
